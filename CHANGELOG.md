...
### Added
- **MiniCPM5-2B** (`mlx-community/MiniCPM5-2B-bf16` + OpenBMB's official `openbmb/MiniCPM5-2B-DSpark`, registry id `minicpm5-2b`). A plain dense `llama`-architecture target (the first llama-type in the registry: mlx-lm's llama module, the generic dense tap, zero model code) with a stock DeepSpec-standalone head (block-7, anchor-as-pos0 — probed: accept 3.1/5.0/5.9 vs 2.0/3.1/3.2 read the other way — reuses the target's embed and lm_head). Measured M4 Pro, 3-trial medians, zero flags (the derived cap is the full block, 7): **3.18× mean — 2.10× chat / 3.16× code / 4.29× math (215 tok/s)**, accept 4.85, baseline 50.5 tok/s; prefill 1640 tok/s at 2k. Greedy-lossless (fp ties only, margins 0.0/0.0/0.125). Confidence head does not pay; lookup drafts a wash (default kept on). Thinking on by default (the template's `<think>` prefill, split into `reasoning_content` / `thinking` blocks as usual; `enable_thinking: false` answers directly).
- **MiniCPM5 tool-call syntax.** The sixth native format the server translates to OpenAI / Anthropic `tool_calls`: `<function name="f"><param name="k">v</param></function>` with CDATA-wrapped values (the model's own chat-template convention; SGLang ships it as the `minicpm5` parser). Schema-typed coercion like the Ornith XML form, truncated calls still parse, and the streaming tool gate holds the opener back like the other five.

### Changed
- **CPU co-prefill runs its CPU share in fp32 through Accelerate BLAS instead of bf16 through BNNS (issue #31, proposed and measured by @felix-ab).** Every SIGSEGV reported for the split (four crashes on two machines, all inside `matmul_bnns` → `BNNSFilterApplyTwoInputBatch` reading an unmapped address) was in BNNS, which mlx uses only for bf16/fp16 CPU GEMMs; fp32 goes to `cblas_sgemm`. The products are identical (a bf16×bf16 product is exact in fp32) and both routes accumulate in fp32, so outputs stay in the same fp-tie band. Cost: some of the prefill win — Qwen3.8-27B-4bit on an M4 Pro, 3-trial medians: split off 136 tok/s, bf16/BNNS 170 / 166 (2k / 4k), fp32/BLAS 157 / 158, i.e. 1.15–1.16× instead of 1.22–1.25× (@felix-ab measured +15–20% on the 6-bit). The fp32 route is calibrated under its own cache key (its balance point differs). `MLX_DSPARK_CPU_SPLIT_FP32=0` restores the bf16/BNNS route for an A/B. The 0.18.1 behaviour of suspending the split for the session on the first memory-pressure shed is kept: under pressure the split's win is not realized and the fp32 operands are larger.
- **Prefix cache (checkpoint mode): a partial hit now inherits the rung ladder below its restore point (issue #36, @felix-ab).** A request restored from a rung checkpoints a slot that carries every rung at or below that rung (shared captures; restores copy). Before, such a slot held no rung below its own boundary, so once the LRU evicted the slot that first planted a static-prefix anchor, every later divergence below it — a new day's date line, a memory edit — was a full cold prefill (130–260 s on a 27B hybrid with a 15–30k-token agent prefix; 2–12 s with the ladder inherited, per the report).
...
### Fixed
- **muse_glimmer: Chat Completions streaming with `tools`, and the Responses stream, leaked the raw `to=self` analysis and `<|eom|>` channel markers into `content` (issue #34, fixed by PR #35, @kangzj).** Those two streaming paths built the `<think>` splitter regardless of model; one helper now picks the muse channel parser for muse on every streaming path, and the answer channel still goes through the tool gate. Agents that always send tools (pi, opencode) showed every muse turn's reasoning as answer text.
- **A DSpark head whose config declares `draft_vocab_size` equal to `vocab_size` no longer fails to load.** The stock DeepSpec trainer writes the field either way; only a genuinely smaller draft vocab needs a `d2t` table, but the standalone/SpecForge/LFM2 config branches passed the value through verbatim and `load_drafter` refused the checkpoint for lacking `d2t` (`openbmb/MiniCPM5-2B-DSpark` declares 130560 == 130560). Equal sizes now normalize to the full-vocab path, as the speculators translator already did; a real mismatch is still refused.
...
### Added
- **Nanbeige4.2-3B** — the first **looped-depth** target (`model_type: nanbeige`: 22 dense layers applied twice with shared weights, per-loop KV caches, an RMSNorm between loops) with Nanbeige's official DSpark head (`Nanbeige/Nanbeige4.2-3B-DSpark`, SpecForge packaging, block-7, reuses the target's embed and lm_head). Registered on the community bf16 MLX conversion (`MercuriusDream/Nanbeige4.2-3B-mlx-bf16`, tensors byte-verified against the original repo). Measured M4 Pro, 3-trial medians, zero flags (the derived cap is 7): **2.92× mean — 2.02× chat / 2.48× code / 4.25× math (71 tok/s)**, accept 3.96, baseline 16.5 tok/s (the loop makes a bf16 decode step read a ~6B model's weights, which is exactly the regime where the drafter pays). Greedy-lossless (fp ties only). mlx-lm has the nanbeige module on main but no release ships it, so `nanbeige_lm.py` vendors it (MIT, NOTICE) behind a registration shim that self-retires once an installed mlx-lm provides it; the port is verified token-identical to the reference PyTorch modeling on the same weights. The drafter's tap layers index the *unrolled* 44-layer stream, and the tap at the loop boundary is captured after the inter-loop norm — the convention of Nanbeige's own sglang serving fork; capturing pre-norm silently cost ~20% acceptance (accept 3.03 → 2.45), the offset-RMSNorm bug class. Note: these checkpoints carry `config.json: auto_map` pointers, so a fresh download currently needs `--trust-remote-code` (nothing is imported on the mlx-lm route).
- **Prefill progress on `/events` (issue #29, requested by @cryptedx).** A long cold prompt used to look like a hung server: nothing streams for the minutes a 20k-token prefill takes on a 27B. The engine now publishes a non-recorded event per evaluated prefill chunk — `{"type": "prefill", "req", "mode", "processed", "total"}`, positions absolute so they start at the reused prefix length — plus a `"done": true` event on every exit path (success, error, cancel) so a client's progress display always clears. Zero cost by construction: the hook fires after the per-chunk `mx.eval` the chunked prefill has always done to bound activations, so no synchronization is added, and the events never enter round history, acceptance curves, or tok/s aggregates (`/rounds` and `/metrics` are unchanged). Round events don't carry a `type` key, so clients tell them apart cleanly; the Mac app's current decoder already skips them (display lands in an app release). All four generation loops report it (dspark / dflash / lookup / baseline).

### Changed
- **CPU co-prefill hands the CPU stream the HEAD rows of each split matmul (issue #31 hardening).** A report of a deterministic SIGSEGV inside mlx's CPU matmul (`matmul_bnns` → `BNNSFilterApplyTwoInputBatch` reading an unmapped address; M4 Pro 24 GB, macOS 26.5.2, gone with `--cpu-split 0`) is consistent with a BNNS tile read running past the end of an input slice: the CPU's old slice was the *tail* of the activation buffer, ending exactly at the allocation's edge, so an over-read that stays silently inside mapped memory on a larger machine can hit an unmapped gap on a tighter one. Giving the CPU the head rows keeps any over-read inside the buffer (the GPU takes the tail; an out-of-bounds GPU read cannot SIGSEGV the process). Same rows, same kernels, row order preserved by the concat — outputs and speed unchanged (measured). Not reproduced on the 48 GB dev machine despite a varied-width stress at the reporter's fractions; if it recurs on the hardened build the cause is upstream (draft prepared: `drafts/mlx-issue-bnns-cpu-split-sigsegv.md`). `--cpu-split 0` remains the off switch.

### Fixed
- **Non-streaming responses no longer misfile a truncated thinking trace as answer text.** On templates that *prefill* the reasoning opener in the prompt (Qwen3-2507 / qwen3_5 / Qwen3.8 / Nanbeige4.2), a generation cut by `max_tokens` before `</think>` contains no thinking markup at all, so the non-streaming reasoning split returned it as `content` with `reasoning_content` empty — while the streaming paths, which have always taken the prompt-tail hint, filed it correctly. `split_thinking` now takes the same `in_thinking` hint and all three non-streaming surfaces (Chat Completions, `/v1/messages`, `/v1/responses`) pass it. Found by the Nanbeige intake smoke; verified live both ways.
...
### Added
- **`usage.prompt_tokens_details.cached_tokens` on the OpenAI endpoints (PR #9, @joeOGsan).** The prefix cache has always known how many prompt tokens it served per request (`reuse_len`, exposed as `reused_tokens` in `x_mlx_dspark` since v0.15.0), but the standard OpenAI usage field — the one SDKs and agent harnesses read to compute cache-hit rate — stayed empty, so every client observed 0% hits. Now reported on non-streaming `/v1/chat/completions` and `/v1/completions`, and on the streamed final chunk when `stream_options.include_usage` is set. n-best (`n>1`) shares one prefix lookup, so the reuse is reported once rather than summed. Landed from the PR as-is with its tests, on top of the field that already carried its author's name.

- **`MLX_DSPARK_MODEL_DIRS`** — extra folders the engine searches for MLX checkpoints before downloading anything (external drives, `~/models`, a NAS mount; `os.pathsep`-separated, `~` expanded). Each root is tried as `publisher/model`, `publisher_model` and bare `model` dirs, MLX-loadable ones only; they appear in `mlx-dspark models`, `/admin/models` (`source: "model_dirs"`, plus the roots under `model_dirs`) and the app's on-disk list, and are excluded from the "our disk usage" total like LM Studio's copies. `mlx-dspark doctor` / `/doctor` now report every search location in resolution order (`model_dirs`), which is the first thing to check when the engine wants to download something you already have.
- **The Hugging Face cache location is honoured everywhere.** The "installed?" check behind `/admin/models` hardcoded `~/.cache/huggingface/hub`, so a user who moved the cache with `HF_HOME` / `HF_HUB_CACHE` saw hub models as not installed while they loaded fine. It now reads the same constant `huggingface_hub` uses.

### Changed
- **Long-context verify split now sizes its chunks from the model's GQA factor.** mlx's fused vector attention kernel only serves `q_len × (heads/kv_heads) ≤ 32`; wider verifies fall to the unfused path (the long-context cliff). The split probe used a fixed 5-row chunk and searched from q=6, which is right for Qwen3.6/3.8-27B (GQA 6) but sat *on* the cliff for GQA-8 targets (Qwen3.6-35B-A3B: window is q≥5, 4-row chunks) and GQA-16 targets (Muse-Glimmer, Nemotron: window q≥3, 2-row chunks) — so exactly those reported "no cliff" and ran unprotected at depth. `calibrate.split_chunk_rows` now derives the chunk (`32 // gqa`, clamped to 8) and the probe confirms it per chip; M4 Pro re-probe: 27B unchanged, GQA-16 shape 1.6–3.3× per attention call at width 3–8, GQA-8 shape 1.8–2.1× at 5–8. Cache key bumped (`smpv2`) so every machine re-probes once.
- **CPU co-prefill picks the smaller row share on a near-tie.** The split's cost curve is asymmetric (past the balance point the CPU becomes the critical path), and microbench noise of a few percent was enough to land one step past it — two calibrations of one machine picked 0.30 and 0.35 at width 2048, and 0.35 cost 7.5% end-to-end (173 vs 188 tok/s). `wide_gemm.pick_fraction` now takes the smallest fraction within 3% of the fastest, with 5 timing iterations instead of 3. Cached calibrations are not invalidated; delete the `cpusplit` entry in `~/.cache/mlx_dspark/calibration.json` (or pass `--cpu-split 0.30`) if yours reads high.

### Fixed
- **LM Studio models were downloaded a second time on first load (issue #28).** The loader has read LM Studio's `~/.lmstudio/models/<publisher>/<model>` folder since v0.12.4 (issue #12), but the cancellable download preflight (`download.ensure_local`) and the "installed?" check behind `/admin/models` each carried their own copy of the on-disk lookup and never learned it — so a model present only in LM Studio was fetched again into the HF cache (~17 GB in the report) and then loaded from LM Studio anyway. There is now exactly one network-free resolver, `load.local_dir` (explicit path → `~/.cache/mlx_dspark/models` under both naming conventions → LM Studio, MLX-loadable dirs only), used by `_resolve`, the preflight and diagnostics, so "where to load from", "is it installed" and "does it need a download" cannot disagree again. The suggested fix of calling `_resolve` from the preflight was not taken: for a repo that is *not* on disk `_resolve` starts a synchronous, un-cancellable download, which is what `ensure_local` exists to avoid. Thanks to @koc-Z3 for the report and repro, and to @jepgambardella whose PR #18 proposed the same single-resolver shape.
